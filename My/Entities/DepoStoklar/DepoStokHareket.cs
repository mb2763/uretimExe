...
{
    [Table("DepoStokHareket")]
    public class DepoStokHareket
    {
      
        public DepoStokHareket Clone() { return (DepoStokHareket)MemberwiseClone(); }
        ...
        {
            string sql = @"SELECT *,'' as Kontrol FROM DepoStokHareket  WITH (NOLOCK) " + sor;
            return sql;
        }
        public static string GetSelectSqlCodeByFisId(Guid? fisId)
        {
            string sql = $@"SELECT *,'' as Kontrol FROM DepoStokHareket  WITH (NOLOCK) where  FisId='{fisId}'";
            return sql;
        }
        public static string GetInsertSqlCode()
        {
            string sql = @"  IF EXISTS
  (SELECT * FROM dbo.DepoStokHareket  WHERE Id = @Id )
    UPDATE dbo.DepoStokHareket SET
      FisId = @FisId,
    ...
    WHERE Id = @Id
  ELSE
    INSERT INTO dbo.DepoStokHareket  (
      Id,
      FisId,
      GirisCikis,
      Tarih,
