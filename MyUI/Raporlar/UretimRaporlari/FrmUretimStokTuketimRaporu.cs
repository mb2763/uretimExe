...
namespace MyUI.Raporlar.UretimRaporlari {
    public partial class FrmUretimStokTuketimRaporu : MyFrmListe {
        ...
        public FrmUretimStokTuketimRaporu() {
            InitializeComponent();
            EventlerBagla();
        }
        ...
        private void Bagla() {
            ...
            string sor2 = SorguAyarla2() + SorguAyarlaTrh();    // if (!string.IsNullOrEmpty(sor)) {  sor = "where  1 = 1 " + sor;  }
            var rs = _srv.GetListViewInKategoriToplu(sor,sor2);
            if (!rs.Success) {
                Cursor.Current = Cursors.Default;
                MesajHata(rs.Message);
                return;
        ...
        private string SorguAyarla() {
            ...
            t1 = TxtAra.Text.Trim();
            if (!string.IsNullOrEmpty(t1)) {
                sor += $" AND ( ITSHD.StokKodu LIKE('%{t1}%') OR ITSHD.StokAdi LIKE('%{t1}%')  )";
            }
            return sor;
        }
        private string SorguAyarla2() {
            ...
            t1 = TxtAra.Text.Trim();
            if (!string.IsNullOrEmpty(t1)) {
                sor += $" AND ( ITHD.StokKodu LIKE('%{t1}%') OR ITHD.StokAdi LIKE('%{t1}%')  )";
            }
            return sor;
        }
